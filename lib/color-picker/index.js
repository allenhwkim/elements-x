...
class XColorPicker extends HTMLElement {
  ...
  _pickColor(event, canvas, circle, desc, target) {
    ...
    canvas.dispatchEvent(new CustomEvent('x-color-changed', {
      bubbles: true,  detail: {
        r, g, b, h, s, l, 
        hex: `#${this._int2hex(r)}${this._int2hex(g)}${this._int2hex(b)}`
      } 
    }));
    target && (target.value = `#${this._int2hex(r)}${this._int2hex(g)}${this._int2hex(b)}`);
  }
...
if (!customElements.get('x-color-picker')) {
