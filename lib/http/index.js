...
class XHttp extends HTMLElement {
  ...
  connectedCallback() { 
    ...
    if (!this.innerHTML.trim()) { // show loading sign if custom html is not provided
      this.payload = getCustomAttributes(this, ['auto', 'url', 'method', 'debug', 'resptype', 'payload']);
  ...
  fetch(payload) {
    ...
    const url = this.method.toUpperCase() === 'GET' ? this._getUrl(payload) : this._getUrl();
    const body = this.method.toUpperCase() === 'GET' ? undefined : this._getBody(payload);
    ...
    return fetch(url, {method, mode, cache, credentials, redirect, referrerPolicy, headers, body})
      .then(resp => {
        ...
        if (this.respStatusCode) {
          if (resp[this.respStatusCode] > 200) throw resp;
        }
