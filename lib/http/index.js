...
import {setHTML, addCss, removeCss, fireAttrChanged} from '../common/util';
...
class XHttp extends HTMLElement {
  ...
  connectedCallback() { 
    ...
    if (!this.innerHTML) { // show loading sign if custom html is not provided
  ...
  fetch(payload) {
    ...
    this.debug && console.debug('<x-http> fetch starts', {payload, method, headers, body});
    ...
    return fetch(url, {method, mode, cache, credentials, redirect, referrerPolicy, headers, body})
      .then(resp => {
        ...
        this.debug && console.debug('<x-http> fetch success', resp);
        const event = new CustomEvent('x-http-success', { bubbles: true, detail: resp});
        this.dispatchEvent(event);
      }).catch(error =>  {
        this.debug && console.debug('<x-http> fetch error', error);
  ...
  _getUrl(qs = {}) {
    ...
    return qsExists ? this.url + suffix : this.url + '?' + suffix;
  }
