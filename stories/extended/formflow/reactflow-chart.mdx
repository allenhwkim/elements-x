...
# ReactflowChart
The React component used by &lt;x-formflow&gt;.
 ...
 * `onNodClick(node, nodes, edges)` function.  
   e.g. `(node, nodes, edges) => console.info(node)`
 * `onEdgeClick(edge, nodes, edges)` function.  
   e.g. `(edge, nodes, edges) => console.info(edge)`
 * `onInit(reactFlowInstance)` function.  
   e.g. `instance => console.info(instance)`
 * `showImage(blob)` function.  
   e.g. `blob => console.info(blob)`
