...
export function addCss(el, css) {
  ...
  if (!document.querySelector(`style[${selectorName}]`)) {
    ...
    console.log('[elements-x] addCss else condition', selectorName);
  }
}
...
export function removeCss(el) {
  ...
  if (styleElForXElem && numXElements < 1) {
    ...
    document.querySelector(`style[${selectorName}]`).remove();
  } else {
    console.log('[elements-x] removeCss else condition', selectorName);
  }
}
