...
export class XOverlay extends HTMLElement {
  ...
  connectedCallback() {
    ...
    setCustomElementHTMLCss(this, html).then(_ => {
      this._setOverlay();
  ...
  _setXElementTarget() {
    const firstChild = this.querySelector('slot').firstElementChild;
    const is1stXElem = firstChild && firstChild.tagName.match(/^X-/);
    // console.log('>>>>>>>>>>>>>>>>>>>', {firstChild, is1stXElem, triggerEl: this.triggerEl});
  ...
  _onDocClick(event) {
    ...
    if (!event.target.isEqualNode(this.triggerEl) && !this.isEqualNode(closetOverlayEl)) {
  ...
  _setTriggerActions() {
    ...
    this.triggerEl = document.getElementById(triggerId) || this.previousElementSibling;
  ...
  _setTriggerEventListener() {
    ...
    const triggerActions = (this.getAttribute('trigger-actions') || 'click,focus').split(','); 
    const closeActions = (this.getAttribute('close-actions') || 'blur').split(','); 
