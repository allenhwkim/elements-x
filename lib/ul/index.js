...
export class XUl extends HTMLElement {
  ...
  _init() {
    ...
    this.removeEventListener('keydown', keydownHandler); // prepared for DOM change, which calls _init()
    ...
    if (this._dropdown) {
      ...
      this.removeEventListener('mouseleave', mouseLeaveListener); // prepared for DOM change, which calls _init()
    ...
    this.querySelectorAll('li').forEach(el => {
      ...
      el.removeEventListener('click', clickHandler); // prepared for DOM change, which calls _init()
  ...
  _fireSelect(el) {
    ...
    if (el && (el.offsetParent !== null)) { // if visible
      ...
      this.dispatchEvent(new CustomEvent('select', { bubbles: true, detail: el }));
    }
  }
  ...
  _liClickHandler(event) {
    ...
    liEl.classList.contains('x-has-list') ?  this._toggleAriaExpanded(liEl) : this._fireSelect(liEl);
    this._highlightEl(0, liEl);
