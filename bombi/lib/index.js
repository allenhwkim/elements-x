...
const { getEsbuildOptions, rimraf, printEsbuildResult, parseShellCommand, konsole } = util;
...
// delete outdir
options.write && rimraf(path.join(process.cwd(), options.outdir)); 
...
esbuild.build(esbuildOptions).then(esbuildResult => {
  ...
  // run post builds
  options.postBuilds.forEach(func => {
    konsole.info('[bombi] post build, ', func.name);
    func(esbuildResult, options)
  });
});
