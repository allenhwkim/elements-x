...
export class XList extends HTMLElement {
  constructor(...args) {
    ...
    this._listItemVarName; // eash list item object variable name. e.g. country
  ...
  connectedCallback() {
    return setCustomElementHTMLCss(this, null, css).then(_ => {
      const tmplEl = this.querySelector('template');
      ...
      this._template = tmplEl && tmplEl.innerHTML; 
      this._listItemVarName = tmplEl && tmplEl.getAttribute('var-name'); 
  ...
  _initHighlightAndSelect(selected, select1st) {
    if (selected) {
      ...
      firstListItem.classList.add('x-highlighted');
    }
  ...
  _renderWithTemplate(template, list = []) {
    ...
    function getElementFromHTML(html, data, varName) {
      template.match(/\${[^}]+}/g).forEach( expr => {
      ...
      varName && (element[varName] = data);
      return element;
    }

    list.forEach( data => {
      const element = getElementFromHTML(template, data, this._listItemVarName);
