...
class XRoute extends HTMLElement {
  ...
  getRoute(location) {
    return this.routes.find( route => location.match(route.regExp) );
  }

  getRoutes() {
    const routes = Array.from(this.querySelectorAll('[href]')).map(el => {
      const location = el.getAttribute('href');
      // const regExp = location.replace(/\*/g, '.*?');
      const params = (location.match(/:[a-z0-9]+/g) || [])
        .map(el => el.replace(':',''));
      const src = el.getAttribute('src');
      const regExp = location.replace(/:[a-z0-9]+/ig, '(.*)');
