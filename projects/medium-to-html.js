...
class MediumToHtml {
    ...
    try {
      ...
      if (json.success) {
        ...
        const htmls = paragraphs.map(p => {
          return this.processParagraph(p, title, subtitle, url)
        });
        const css = `<style>
  ...
  processParagraph(paragraph, title, subtitle, url) {
    const {name, type, text, markups, metadata, layout, iframe, mixtapeMetadata} = paragraph; 
    const marked = this.processMarkups(text, markups);
    ...
    switch(type) {
      ...
      case 9: // unordered list
        html = `<li class="bulletin">${html}</li>`; break;
      case 10: // ordered list
        html = `<li class="number">${html}</li>`; break;
      ...
      case 4: // image with text
        html = ''+
          `<figure class="layout-${layout}">` +
              `<img src="https://cdn-images-1.medium.com/${metadata.id}" />` +
    ...
    while(match = html.match(/[θ]+/)) {
      ...
      html = html.substr(0, start) + marked.replacements.shift() + html.substr(start + length);
    }
    return html;
  }

  processMarkups(text, markups) {
