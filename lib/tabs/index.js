...
import { addCss, removeCss, define } from '../common';

export class XTabs extends HTMLElement {
  // adoptedCallback() {}
  ...
  _selectTabAndContents(tabId, setFocus=true) {
    if (!tabId) {
      const tabEl =
        this.querySelector('[tab-for][selected]') || // user-defined
  ...
  _selectTab(tabId, setFocus) {
    ...
    this.tabsEls.filter(el => !el.isEqualNode(tabEl))
      .forEach(el => {
        el.removeAttribute('selected'); // new way
        el.removeAttribute('tabindex');
      });

    tabEl.setAttribute('selected', 'true'); // new way
  ...
  _selectContents(tabId) {
    ...
    if (contentsEl) {
      ...
      contentsEl.setAttribute('selected', ''); // new way
    }
  }
...
XTabs.define = define('x-tabs', XTabs);
