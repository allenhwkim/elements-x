...
export function parseCommandExpr(el, commandExpr, scope) {
  ...
  let options; 
  // the following logic determines `options`, which is not set yet.
  ...
  } else if (firstOptionExpr.match(/^['"].+['"]$/)) { // if quoted, which is a string
    const str = firstOptionExpr.replace(/^['"]/, '').replace(/['"]$/, '');
    options = [str];
