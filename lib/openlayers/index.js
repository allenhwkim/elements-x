import { waitUntil, setHTML, addCss, removeCss, define } from '../common/util';
import css from './openlayers.css';
import html from './openlayers.html';
import { getLonLat } from './get-lon-lat';
export { XOLMarker } from './x-ol-marker.js';
...
export class XOpenLayers extends HTMLElement {
  ...
  connectedCallback() {
    ...
    setTimeout(_ => { // trying delay for a framework like Angular
      this.zoom = +this.getAttribute('zoom') || 13;
      this.center = this.getAttribute('center');
      this.satellite = this.getAttribute('satellite') !== null;
      setHTML(this, html)
        .then(_ => waitUntil(_ => this.querySelector('.map').offsetWidth > 0))
        .then(_ => waitUntil('ol'))
        .then(ol => {
          this.map = new window.ol.Map({ target: this.querySelector('.map') });
          this.map.addLayer(new window.ol.layer.Group({ layers: this.getLayers() }));
  ...
  getCustomControl(ol) {
    class LayerControl extends window.ol.control.Control {
      constructor(options = {}) {
      ...
      handleRotateNorth() {
        const layers = this.getMap().getLayerGroup().getLayersArray()
          .filter(layer => layer instanceof window.ol.layer.Tile);
  ...
  getLayers() {
    const layers = [
      new window.ol.layer.Tile({ source: new window.ol.source.XYZ({
...
XOpenLayers.define = define('x-openlayers', XOpenLayers);
