...
import {addCss, removeCss, setHTML, waitUntilIdle, define} from '../common/util';
...
export class XTable extends HTMLElement {
  ...
  set list(val) {
    ...
    if (paginationEl) {
      paginationEl.numRecordsPerPage = this._limit;
      paginationEl.totalRecords = this.list.length;
      paginationEl._reset();
    }
  }
  ...
  connectedCallback() {
    ...
    waitUntilIdle(this)
      .then(_ => setHTML(this, html))
      .then(_ => {
        const tmplSuccess = this.getAttribute('list-from') === null ?
          this._getTemplateSuccess() :
          this.querySelector('tbody > tr:not(.error):not(.empty):not(.bottom)').outerHTML;
        const success = this._getMobileTemplateSuccess(tmplSuccess);
        const error = (this.querySelector('tbody > tr.error') || {}).outerHTML ||
          '<tr><td colspan="100">Unknown error</td></tr>';
        const empty = (this.querySelector('tbody > tr.empty') || {}).outerHTML ||
          '<tr><td colspan="100">No records found</td></tr>';
        const bottom = (this.querySelector('tbody > tr.bottom') || {}).outerHTML ||
            '<tr><td colspan="100">Bottom of list</td></tr>';
        this._template = {success, error, empty, bottom};
  ...
  _setList() {
    ...
    if (listFrom && listFrom.tagName === 'X-HTTP') {
      listFrom.resp && (this.list = listFrom.resp);
      ...
      this.list = this._getListFromTbody();
    }
  }
  ...
  _setOrderBy() {
    this.querySelector('table thead tr').addEventListener('click', e => {
      const column = e.target.getAttribute('data-key') || e.target.className.split(' ')[0];
  ...
  _getKeysFromThead() {
    const keys = [];
    this.querySelectorAll('thead > tr > *').forEach( el=> {
      const key =  (el.className && el.className.split(' ')[0]) ||
        el.innerText.replace(/[^a-z0-9_]/ig, '_');
  ...
  _getTemplateSuccess() {
    const keys = this._getKeysFromThead();
    const tr = this.querySelector('tbody > tr');
    if (tr) {
      let template = tr.outerHTML; 
      const tds = tr.outerHTML.match(/<td.*?>.*?<\/td>/g);
      tds.forEach((td, index) => {
        const [_, m1, m2, m3] = td.match(/(<td.*?>)(.*)(<\/td>)/); // eslint-disable-line no-unused-vars
        template = template.replace(td, `${m1}{{${keys[index]}}}${m3}`);
      });
      return template;
    } else {
      return '';
  ...
  // do we need this?
  _getMobileTemplateSuccess(html) {
    const keys = this._getKeysFromThead();
    const tds = html.match(/<td(.*?)>.*?<\/td>/g);
    tds.forEach((td, index) => {
      const [_, m1, m2] = td.match(/<td(.*?)>(.*?)<\/td>/);
      html = html.replace(td, `<td${m1} data-label="${keys[index]}">${m2}</td>`);
    });
    return html;
  }
  ...
  _getListFromTbody() {
    ...
    trs.forEach(tr => {
      ...
      tr.querySelectorAll('td').forEach( (td, index) => {
        listItem[keys[index]] = td.innerText;
      });
      list.push(listItem);
    });
    return list;
...
XTable.define = define('x-table', XTable);
