...
export function resetCalendar (el) {
  ...
  function fireDateSelectedEvent(date) {
    const custEvent = new CustomEvent('x-date-selected', {bubbles: true, detail: date});
    el.dispatchEvent(custEvent);
    el.target && setTargetValue(el.target, date);
  }
  ...
  function date2str(date, format='yyyy-MM-dd') {
    ...
    const formatted = format
      .replace(/(y+)/g, v => date.getFullYear().toString().slice(-v.length))
  ...
  if (dates) {
    dates.forEach(date => {
      ...
      if (isSameDate(date, selectedDate)) {
        ...
        // TODO; set date format here YYYY-MM-DD
        selectedDate.toString = function() { return date2str(selectedDate, el._dateFormat); };
        fireDateSelectedEvent(selectedDate);
        // const custEvent = new CustomEvent('x-date-selected', {bubbles: true, detail: selectedDate});
        // el.dispatchEvent(custEvent);
      }
      ...
      $('.x-days-container') && $('.x-days-container').appendChild(dateEl);
    });
  }
}