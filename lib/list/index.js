...
export class XList extends HTMLElement {
  constructor(...args) {
    ...
    this._selected;
    this._css = css;
    this._filterBy; // input element id that limits display of children
    return self;
  }

  connectedCallback() {
    return setCustomElementHTMLCss(this, null, this._css).then(_ => {
      ...
      // If it's under x-overlay with trigger element, also listen to triggerEl keyboard events.
      this._setTriggerElKeydownHandler(); 
    })
  }
  ...
  _init() {
    ...
    if (this._filterBy) {
      const inputEl = document.getElementById(this._filterBy);
      if (!inputEl) {
        console.error('Invalid filter-by', this._filterBy);
        return false;
      }
      ...
      this.addEventListener('x-listitem-selected', event => {
        ...
        inputEl.value = event.detail.innerText;
        inputEl.innerText = event.detail.innerText;
        const xOverlayContainer = this.closest('x-overlay');
        xOverlayContainer.close();
      });
    }
  }
  ...
  select() {
    ...
    } else if (this._selectFirst && (this.querySelector('li') || this.firstElementChild)) {
  ...
  _setTriggerElKeydownHandler() {
    const xOverlayContainer = this.closest('x-overlay');
    if (xOverlayContainer && xOverlayContainer ._triggerEl) {
      const triggerEl = xOverlayContainer._triggerEl;
      ...
      triggerEl.addEventListener('input', event => {
