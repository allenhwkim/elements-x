...
export function resetCalendar (el) {
  ...
  const [curDate, minDate, maxDate] = [el._curDate, el._minDate, el._maxDate];
  ...
  dates.forEach(date => {
    ...
    isSameDate(date, selectedDate) && dateEl.classList.add('x-selected');
    ...
    dateEl.insertAdjacentHTML('beforeend', 
