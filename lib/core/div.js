...
export function getAttrExprValue(scope, expr) {  // expr is attribute value, e.g. x-value="todos.length"
...
// add(todos, id=d3s3x2, id=123, foo=bar) -> todos, [id=d3s3x2, id=123, foo=bar]
// add(todos, name=::value, id=::id, done=false) -> todos, [name=::value, id=::id, done=false]
// set(todos, q=#search, limit=#limit) -> todos, [q=#search, limit=#limit]
// set(#my-id, foo) -> #my-id, [foo]
// set(foo, #my-section .my-klass) -> foo, ['#my-section .my-klass']
// set(foo, my name is {{name}}) -> foo, ['my name is {{name}}']
export function getCommandArgs(el, expressions) { // target is DOM element or scoped property
  ...
  if (exprs.length === 0) {
    if (typeof el.value !== 'undefined') {
      debug && console.info('getArgs case 1', exprs.length, target.value);
      return el.value;
    } else {
