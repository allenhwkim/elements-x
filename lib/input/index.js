import {removeCss, define, syncAttr } from '../common';
...
export class XInput extends HTMLElement {
  ...
  connectedCallback() {
    setTimeout( _ => { // in a framework like Angular, attribute set slowly
      const type = this.getAttribute('type') ? this.getAttribute('type') :
        this.getAttribute('mask') ? 'mask' : 'text';
      !this.getAttribute('type') && this.setAttribute('type', type);
      ...
      types[type].init.bind(this)().then(_ => {
        ...
        if (isTextInput && isTextField ) {
          const xInputEl = this.querySelector("x-input");
          xInputEl && syncAttr(this, xInputEl, ['type']);
...
XInput.define = define('x-input', XInput);
