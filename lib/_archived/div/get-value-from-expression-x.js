...
export function getValueFromExpressionX(scope, expr) { 
  ...
  if (Array.isArray(expr) && expr.length === 3) { // [true, 'On', 'Off']
  ...
  // eslint-disable-next-line no-useless-escape
  } else if ((expr.match(/ /) || expr.match(/[^a-z0-9_.\[\]]/i)) && !expr.includes('=')) { // 'string val'
    dbg && console.info('  getValueFromExpressionX string', {expr});
    (expr.match(/[{\[]{2}.+[}\]]{2}/g) || []).forEach(el => {
    ...
    return '';
  } else if (typeof scope[expr] !== 'undefined') { // if string, and it's a scope obj
    dbg && console.info('  getValueFromExpressionX scope expression', {expr});
    return scope[expr];
  } else {
