...
export class XList extends HTMLElement {
  ...
  constructor(...args) {
    ...
    this.xHttp; // <x-http> linked to list. if given, when x-success event provides list value
    this.xPagination; // <x-pagination> linked to list. if given, x-select event provides _offset value
    return self;
  }
  ...
  _handlePagination() {
    ...
    this.xPagination && (this.value.length !== this._limit) &&
      this.xPagination.addEventListener('x-select', event => {
  ...
  _handleHttpResponse() {
    ...
    this.xHttp && this.xHttp.addEventListener('x-success', event => {
  ...
  _renderWithTemplate(reset = true) {
    ...
    if (this._limit >= list.length) { // paginated api data, show as it is 
      // no action required
    } else if (this._offset && this._limit) {  // big list, showing sliced
