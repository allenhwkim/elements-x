import './marker.js';
import {setHTML, addCss, removeCss, define} from '../common';
import css from './mapbox.css';

function getAccessToken(token) {
  return token || window.mapboxgl?.accessToken || XMapbox.accessToken || window.xKeys?.mapbox;
}
...
export function getLngLat(str) {
  return new Promise( (resolve, reject) => {
    if (typeof str === 'string' && str.match(/[-0-9.]+,\s?[-0-9.]+/)) { //'-99.99, 99'
      ...
      const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${str}.json?` +
        `types=address&access_token=${getAccessToken()}`;
...
export class XMapbox extends HTMLElement {
  ...
  _init() {
    ...
    return this._setLibrary().then(
      resp => { // resp is `window.mapboxgl`
        !window.mapboxgl.accessToken && (window.mapboxgl.accessToken = getAccessToken(accessToken));
...
XMapbox.define = define('x-mapbox', XMapbox);
