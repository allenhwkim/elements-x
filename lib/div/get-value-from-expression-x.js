...
export function getValueFromExpressionX(scope, expr) { 
  ...
  if (expr === '::id') { // a unique id
    ...
    dbg && console.info('  getValueFromExpressionX boolean', {expr});
    return (''+expr) === 'true';
  ...
  } else if (expr.match(/^['"{]/) || expr.match(/ /)) { // 'string val'
    ...
    if (exprs.length === 0) { // e.g., todos
      ...
      dbg && console.info('  getValueFromExpressionX endsWithPropName', {propName, exprs});
      ...
      exprs.forEach(expr => {
        ...
        targetArr = targetArr.filter(el =>  {
          dbg && console.info('  getValueFromExpressionX endsWithPropName', el, {key, value});
      ...
      dbg && console.info('  getValueFromExpressionX endsWithCondition', {propName, exprs});
      let targetArr = [...scope[propName]];
      exprs.forEach(expr => {
        const [key, value] = expr.split('=');
