...
export class XTable extends HTMLElement {
  ...
  constructor(...args) {
    ...
    this.xPagination; // <x-pagination> linked to list. if given, x-page-selected event provides _offse 
  ...
  _setupPagination() {
    this.querySelector('x-pagination').addEventListener('x-page-selected', event => {
  ...
  _setListFrom() {
    ...
    if (listFrom?.tagName === 'X-HTTP') {
      listFrom.addEventListener('x-http-success', event => {
