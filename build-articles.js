...
const fs = require('fs');
const MediumToHtml = require('./projects/medium-to-html');
...
const promises = urls.map(url => {
  return med2html.getHtml(url + '?format=json')
    .then(({json, html}) => {
      ...
      return {id, title, mediumUrl, uniqueSlug, html};
    });
});
...
Promise.all(promises).then(all => {
  ...
  all.forEach(el => {
    const {id, title, mediumUrl, uniqueSlug, html} = el;
    index[id] = {id, title, mediumUrl, uniqueSlug};
