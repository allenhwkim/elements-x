...
export function set(scope, target, source) {
  ...
  scope.dbg && console.info('  set', {scope, target, source});
  const {refs, propName, attrName} = getTargetFromExpressionX(scope, target);
  scope.dbg && console.info('  set getTargetFromExpressionX', {refs, propName, attrName});
  const sourceVal = getValueFromExpressionX(scope, source);
  scope.dbg && console.info('  set', {scopePropName, sourceVal});
  ...
  refs.forEach(ref => {
    if ((ref instanceof HTMLElement) && attrName)  {
      ...
      scope.dbg && console.info('  setting array elment', {ref, propName, ndx, sourceVal});
...
export function remove(scope, target, params) {
  scope.dbg && console.info('  remove', {scope, target, params});
  if (target.match(/^[.#]/)) { // DOM element, x-click=“delete(#todos[filter])“
    ...
    const [scopePropName, ..._] = target.split('.');
    refs.forEach(ref => {
      scope.dbg && console.info('  remove object', {ref, propName});
