...
export class TranslationElement extends HTMLElement {
  constructor() {
    super();
  ...
  connectedCallback() {
    ...
    if (lang !== 'en' && !Translation[lang]) {
      console.error('Please provide translation before app starts');
    }
  ...
  _translate(keyEl) {
    ...
    const attrMap = this._getAttrMap();
    const translated = Translation.translate(key, attrMap);
    if (translated) {
...
if (!customElements.get('t-9n')) {
  setTimeout(_ => customElements.define('t-9n', TranslationElement));
}